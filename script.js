/* ============================================================
   HEIDE PARK ROBLOX — script.js  v5 (Final Polish)
   Live status polling · 7-step booking · GT competition sim
   ============================================================ */
'use strict';

// ── State ──────────────────────────────────────────────────
let currentUser = null;
let currentAge  = null;
let calYear, calMonth;
let captchaAnswer = 0;
let activeDate    = null;   // { key, d, bookedBy }
let statusPollTimer = null; // interval handle for live status updates
let gtCompTimer     = null; // golden ticket competition animation

// ── Park zones (only 2) ──────────────────────────────────
const ZONES = [
  { icon:'🎢', name:'Thunder Zone',    desc:'High-speed coasters, intense drops, extreme thrills',  grad:'linear-gradient(145deg,#6a1200,#c83010)' },
  { icon:'🌊', name:'Aqua World',      desc:'Water slides, river rapids & full splash zones',        grad:'linear-gradient(145deg,#00274a,#0060a8)' },
];

const MONTHS = ['January','February','March','April','May','June','July','August','September','October','November','December'];
const MON_S  = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];

// ── LocalStorage ──────────────────────────────────────────
function lsKey(u){ return 'hpr_' + u.toLowerCase(); }
function userData(u){
  try { const r=localStorage.getItem(lsKey(u)); if(r) return JSON.parse(r); } catch(e){}
  return { age:null, pendingDates:[], goldenPending:false };
}
function saveData(u,d){ try{ localStorage.setItem(lsKey(u),JSON.stringify(d)); }catch(e){} }
function isPendingDate(u,k){ return userData(u).pendingDates.includes(k); }
function isBookedDate(u,k){
  const [y,mo,d]=k.split('-').map(Number);
  const cfg=((SETTINGS.bookingDays[y]||{})[mo]||{})[d];
  return !!(cfg&&(cfg.bookedBy||[]).some(x=>x.toLowerCase()===u.toLowerCase()));
}
function isGoldenPending(u){ return userData(u).goldenPending===true; }
function isGoldenConfirmed(u){
  const gt=SETTINGS.goldenTicket;
  return !!(gt.claimedBy&&gt.claimedBy.toLowerCase()===u.toLowerCase());
}

/* ============================================================
   BOOT
   ============================================================ */
window.addEventListener('DOMContentLoaded', () => {
  // Page loader
  const fill=document.getElementById('pl-fill'), msg=document.getElementById('pl-msg');
  const lMsgs=['Initialising…','Loading event data…','Preparing calendar…','Almost ready…'];
  let p=0, mi=0;
  const li=setInterval(()=>{
    p=Math.min(p+(Math.random()*18+6),100);
    if(fill) fill.style.width=p+'%';
    mi=Math.min(mi+1,lMsgs.length-1);
    if(msg) msg.textContent=lMsgs[mi];
    if(p>=100){ clearInterval(li); setTimeout(()=>document.getElementById('page-loader')?.classList.add('out'),200); }
  },320);

  const now=new Date(); calYear=now.getFullYear(); calMonth=now.getMonth()+1;

  initCanvas();
  buildTicker();
  buildCalSelects();
  renderCal();
  buildGoldenTicket();
  initScrollReveal();
  wireNav();
  wireBookingFlow();
  wireGoldenFlow();
  document.getElementById('btn-logout')?.addEventListener('click', doLogout);
});

/* ============================================================
   LIVE STATUS POLLING
   Runs every 15 seconds when logged in.
   Re-reads SETTINGS (which may have been updated) and
   refreshes calendar + golden ticket + pending badge.
   On first call it also checks if any previously "pending"
   dates in localStorage are now confirmed in settings.js.
   ============================================================ */
function startStatusPolling() {
  stopStatusPolling();
  // Run immediately once on login
  pollStatus();
  statusPollTimer = setInterval(pollStatus, 15000);
}

function stopStatusPolling() {
  if (statusPollTimer) { clearInterval(statusPollTimer); statusPollTimer=null; }
}

function pollStatus() {
  if (!currentUser) return;
  // Check if any pending dates are now confirmed in SETTINGS
  const d = userData(currentUser);
  let changed = false;
  d.pendingDates = d.pendingDates.filter(dk => {
    // Keep pending if NOT yet in bookedBy; if it IS confirmed, keep it
    // (we don't remove from pending — the user might want to see their history)
    return true;
  });
  // Silently re-render everything
  renderCal();
  buildGoldenTicket();
  updatePendingBadge();
}

/* ============================================================
   LOGIN CANVAS
   ============================================================ */
function initCanvas() {
  const c=document.getElementById('lc'); if(!c) return;
  const ctx=c.getContext('2d'); let W,H;
  const pts=[];
  function resize(){ W=c.width=innerWidth; H=c.height=innerHeight; }
  resize(); window.addEventListener('resize',resize);
  for(let i=0;i<60;i++) pts.push({
    x:Math.random()*1920,y:Math.random()*1080,
    r:Math.random()*1.8+.4,vx:(Math.random()-.5)*.35,vy:(Math.random()-.5)*.35,
    a:Math.random()*.4+.06,c:['#c8380a','#e85020','#8a2200'][Math.floor(Math.random()*3)]
  });
  (function tick(){
    ctx.clearRect(0,0,W,H);
    pts.forEach(p=>{
      p.x=(p.x+p.vx+W)%W; p.y=(p.y+p.vy+H)%H;
      ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);
      ctx.fillStyle=p.c;ctx.globalAlpha=p.a;ctx.fill();
    });
    ctx.globalAlpha=1; requestAnimationFrame(tick);
  })();
}

/* ============================================================
   LOGIN / LOGOUT
   ============================================================ */
document.getElementById('login-form').addEventListener('submit', function(e) {
  e.preventDefault();
  const u=document.getElementById('l-user').value.trim();
  const a=parseInt(document.getElementById('l-age').value);
  const t=document.getElementById('l-terms').checked;
  document.getElementById('login-error').style.display='none';
  if(!u||u.length<3) return loginErr('Please enter a valid Roblox username (min 3 chars).');
  if(!a||a<5||a>99)  return loginErr('Please enter a valid age (5–99).');
  if(!t)             return loginErr('You must accept the Terms of Service to continue.');

  currentUser=u; currentAge=a;
  const saved=userData(u);
  if(!saved.age){ saved.age=a; saveData(u,saved); }

  document.getElementById('login-screen').classList.add('out');
  document.getElementById('site').classList.add('on');
  document.getElementById('sh-avatar').textContent=u.charAt(0).toUpperCase();
  document.getElementById('sh-uname').textContent=u;

  document.querySelectorAll('.section').forEach(s=>s.classList.add('on'));
  renderCal();
  buildGoldenTicket();
  updatePendingBadge();
  checkGoldenNav();
  startStatusPolling();
  startGTCompetition();
});

function loginErr(m){
  const el=document.getElementById('login-error');
  el.textContent=m; el.style.display='block';
  el.style.animation='none'; void el.offsetWidth; el.style.animation='shake .38s ease';
}

function doLogout(){
  currentUser=null; currentAge=null;
  stopStatusPolling(); stopGTCompetition();
  document.getElementById('site').classList.remove('on');
  document.getElementById('login-screen').classList.remove('out');
  document.getElementById('l-user').value='';
  document.getElementById('l-age').value='';
  document.getElementById('l-terms').checked=false;
  document.getElementById('login-error').style.display='none';
  document.getElementById('sh-avatar').textContent='?';
  document.getElementById('sh-uname').textContent='Guest';
  document.getElementById('sh-pending').style.display='none';
}

function updatePendingBadge(){
  if(!currentUser) return;
  const d=userData(currentUser);
  // Also check if golden is actually confirmed now
  const gtPend = d.goldenPending && !isGoldenConfirmed(currentUser);
  const hasPend = d.pendingDates.some(k => !isBookedDate(currentUser,k)) || gtPend;
  document.getElementById('sh-pending').style.display = hasPend ? 'block':'none';
}

function checkGoldenNav(){
  const gt=SETTINGS.goldenTicket;
  if(gt&&gt.enabled) document.getElementById('nav-golden').style.display='';
}

/* ============================================================
   NAV
   ============================================================ */
function wireNav(){
  const sc=id=>e=>{e.preventDefault();document.getElementById(id)?.scrollIntoView({behavior:'smooth'});};
  document.getElementById('nav-tickets')?.addEventListener('click',sc('calendar-section'));
  document.getElementById('nav-golden')?.addEventListener('click',sc('golden-ticket-section'));
  document.getElementById('hero-book-btn')?.addEventListener('click',sc('calendar-section'));
  document.getElementById('hero-park-btn')?.addEventListener('click',sc('golden-ticket-section'));
}

/* ============================================================
   TICKER
   ============================================================ */
function buildTicker(){
  const el=document.getElementById('ticker-inner'); if(!el) return;
  const news=SETTINGS.parkNews||[];
  if(!news.length){ document.querySelector('.ticker-bar')?.remove(); return; }
  let html='';
  for(let p=0;p<3;p++) news.forEach(n=>html+=`<span>${n.date} &ensp;${n.text}</span>`);
  el.innerHTML=html;
}

/* ============================================================
   GOLDEN TICKET + COMPETITION SIMULATION
   ============================================================ */
// Fake player name pool for the "others watching" display
const FAKE_NAMES = [
  'Xer0Storm','BlazePvP','NightRacer','CoasterKing',
  'VoidWalker','ProRider99','ShadowDash','ArcadeHero',
  'TurboFly','CoolSurfer','DarkWave','ZenithX',
  'SpeedRun77','NeonBolt','StarCraft','RocketJr',
  'ThunderHawk','IronWill','GhostBlaze','FrostByte'
];

function startGTCompetition(){
  stopGTCompetition();
  if(!SETTINGS.goldenTicket?.enabled) return;
  if(SETTINGS.goldenTicket.claimedBy) return;
  updateGTCompetition();
  gtCompTimer=setInterval(updateGTCompetition, 3500);
}

function stopGTCompetition(){
  if(gtCompTimer){ clearInterval(gtCompTimer); gtCompTimer=null; }
}

function updateGTCompetition(){
  const box=document.getElementById('gt-live-box'); if(!box) return;
  const gt=SETTINGS.goldenTicket;
  if(!gt||!gt.enabled||gt.claimedBy){ box.style.display='none'; stopGTCompetition(); return; }

  // Generate believable viewer count that fluctuates
  const base = 8 + Math.floor(Math.random()*14);
  const count = base + Math.floor(Date.now()/8000 % 5);
  document.getElementById('gt-viewer-count').textContent = `${count} players watching`;

  // Pick 5 random fake avatars
  const avatarEl = document.getElementById('gt-avatars');
  if(avatarEl){
    const picked = [];
    const pool = [...FAKE_NAMES];
    for(let i=0;i<5;i++){
      const idx=Math.floor(Math.random()*pool.length);
      picked.push(pool.splice(idx,1)[0]);
    }
    avatarEl.innerHTML = picked.map((n,i)=>`
      <div class=\"gt-av\" style=\"--delay:${i*0.12}s\" title=\"${n}\">
        ${n.charAt(0).toUpperCase()}
      </div>`).join('');
  }

  // Rotating urgency messages
  const urgMsgs = [
    'Players are competing for this ticket right now',
    `${count-2} others are trying to claim it`,
    'This is the only one — act fast',
    'Once claimed, it\'s gone forever',
    `${Math.floor(Math.random()*3)+2} requests already sent today`,
  ];
  const msgEl=document.getElementById('gt-live-msg');
  if(msgEl) msgEl.textContent=urgMsgs[Math.floor(Math.random()*urgMsgs.length)];

  box.style.display='';
}

function buildGoldenTicket(){
  const sec=document.getElementById('golden-ticket-section'); if(!sec) return;
  const gt=SETTINGS.goldenTicket;
  if(!gt||!gt.enabled){ sec.style.display='none'; return; }
  sec.style.display='';

  const confirmed = !!(gt.claimedBy&&gt.claimedBy.trim());
  const byMe      = currentUser&&isGoldenConfirmed(currentUser);
  const pendMe    = currentUser&&isGoldenPending(currentUser);

  const db=document.getElementById('gt-date-badge');
  if(db&&gt.bookingDate){
    db.innerHTML=`📅 Valid: <strong>${gt.bookingDate}</strong> · ${gt.label||'Golden Event'}`;
    db.style.display='inline-flex';
  }

  const claimBtn=document.getElementById('btn-claim-golden');
  const pendBdg=document.getElementById('gt-pending-badge');
  const clmBdg=document.getElementById('gt-claimed-badge');
  const liveBox=document.getElementById('gt-live-box');

  [claimBtn,pendBdg,clmBdg].forEach(el=>{if(el)el.style.display='none';});
  if(liveBox) liveBox.style.display='none';

  if(byMe){
    if(clmBdg){ clmBdg.innerHTML='✅ You hold this ticket!'; clmBdg.style.display='inline-flex'; }
    stopGTCompetition();
  } else if(confirmed){
    if(clmBdg){ clmBdg.innerHTML=`🔒 Claimed by <strong style="margin-left:4px">${gt.claimedBy}</strong>`; clmBdg.style.display='inline-flex'; }
    stopGTCompetition();
  } else if(pendMe){
    if(pendBdg) pendBdg.style.display='inline-flex';
    if(liveBox) liveBox.style.display='none';
  } else {
    if(claimBtn) claimBtn.style.display='inline-flex';
    if(liveBox) liveBox.style.display='';
    if(currentUser) startGTCompetition();
  }
}

/* ============================================================
   CALENDAR SELECTS
   ============================================================ */
function buildCalSelects(){
  const ms=document.getElementById('cal-month'), ys=document.getElementById('cal-year');
  if(!ms||!ys) return;
  const now=new Date();
  MONTHS.forEach((m,i)=>{
    const o=document.createElement('option'); o.value=i+1; o.textContent=m;
    if(i+1===calMonth) o.selected=true; ms.appendChild(o);
  });
  for(let y=now.getFullYear();y<=now.getFullYear()+2;y++){
    const o=document.createElement('option'); o.value=y; o.textContent=y;
    if(y===calYear) o.selected=true; ys.appendChild(o);
  }
  ms.addEventListener('change',()=>{ calMonth=+ms.value; renderCal(); });
  ys.addEventListener('change',()=>{ calYear=+ys.value;  renderCal(); });
  const maxEl=document.getElementById('cal-max');
  if(maxEl) maxEl.textContent=SETTINGS.park.maxBookingsPerDay;
}

/* ============================================================
   CALENDAR RENDER
   ============================================================ */
function renderCal(){
  const grid=document.getElementById('calendar-grid'); if(!grid) return;
  grid.innerHTML='';

  const today=new Date(); today.setHours(0,0,0,0);
  const maxDate=new Date(today); maxDate.setDate(maxDate.getDate()+SETTINGS.park.bookingWindowDays);
  const first  =new Date(calYear,calMonth-1,1).getDay();
  const daysInM=new Date(calYear,calMonth,0).getDate();
  const mCfg   =((SETTINGS.bookingDays[calYear]||{})[calMonth])||{};
  const MAX    =SETTINGS.park.maxBookingsPerDay;
  const gt     =SETTINGS.goldenTicket;

  // Empty offset cells
  for(let i=0;i<first;i++){
    const c=document.createElement('div'); c.className='cal-day empty'; grid.appendChild(c);
  }

  for(let d=1;d<=daysInM;d++){
    const dt=new Date(calYear,calMonth-1,d); dt.setHours(0,0,0,0);
    const dk=`${calYear}-${String(calMonth).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
    const cfg    =mCfg[d];
    const enabled=!!(cfg&&cfg.enabled);
    const bookedBy=enabled?(cfg.bookedBy||[]):[];
    const soldOut =bookedBy.length>=MAX;
    const isPast  =dt<today;
    const inWin   =dt>=today&&dt<=maxDate;
    const isGolden=!!(gt&&gt.enabled&&gt.bookingDate===dk);
    const isBooked=!!(currentUser&&isBookedDate(currentUser,dk));
    const isPend  =!!(currentUser&&isPendingDate(currentUser,dk));

    let cls='cal-day', label='', clickable=false;

    if(isPast){
      cls+=' past';
    } else if(!enabled){
      cls+=' off';
    } else if(!inWin){
      cls+=' far';
    } else if(isBooked){
      cls+=' confirmed'; label='Booked ✓';
    } else if(isPend){
      cls+=' pend-day'; label='Pending ⏳';
    } else if(soldOut){
      cls+=' soldout'; label='Full';
    } else if(bookedBy.length>0){
      const open=MAX-bookedBy.length;
      cls+=' partial'; label=`${open} left`; clickable=true;
    } else {
      cls+=' available'; label='Available'; clickable=true;
    }

    if(isGolden) cls+=' golden-day';

    const cell=document.createElement('div'); cell.className=cls;
    // Tooltip for the hover
    if(enabled&&!isPast) cell.title=isBooked?'Your confirmed booking'
      :isPend?'Your pending request'
      :soldOut?'This date is fully booked'
      :`${bookedBy.length} of ${MAX} spots taken`;

    cell.innerHTML=`
      ${isGolden?'<div class="cal-gdot">✦</div>':''}
      <span class="dn">${d}</span>
      ${label?`<span class="dl">${label}</span>`:''}
      ${!isPast&&enabled&&inWin&&!soldOut&&!isBooked&&!isPend?`<div class="cal-hover-ring"></div>`:''}
    `;
    if(clickable) cell.addEventListener('click',()=>openBooking(dk,d,bookedBy));
    grid.appendChild(cell);
  }
}

/* ============================================================
   BOOKING FLOW — 7 steps
   s0: connection loading
   s1: identity + captcha
   s2: rules + checkboxes
   s3: review summary
   s4: transmission loading (new)
   s5: final verification (new)
   s6: done
   ============================================================ */
function wireBookingFlow(){
  document.getElementById('bk-x')?.addEventListener('click',closeBooking);
  document.getElementById('bk-overlay')?.addEventListener('click',e=>{
    if(e.target===document.getElementById('bk-overlay')) closeBooking();
  });
  document.getElementById('bk-n1')?.addEventListener('click',step1to2);
  document.getElementById('bk-n2')?.addEventListener('click',step2to3);
  document.getElementById('bk-b2')?.addEventListener('click',()=>showStep('bk-s1'));
  document.getElementById('bk-b3')?.addEventListener('click',()=>showStep('bk-s2'));
  document.getElementById('bk-submit')?.addEventListener('click',doSubmit);
  document.getElementById('bk-done-close')?.addEventListener('click',closeBooking);
}

function showStep(id){
  document.querySelectorAll('#bk-overlay .bk-step').forEach(s=>{
    s.classList.remove('active'); s.style.display='none';
  });
  const el=document.getElementById(id);
  if(el){ el.style.display='block'; el.classList.add('active'); }
}

function openBooking(dk,d,bookedBy){
  if(!currentUser) return;
  activeDate={key:dk,d,bookedBy};

  // Reset fields
  document.getElementById('bk-uname').value='';
  document.getElementById('bk-cap-a').value='';
  ['cb1','cb2','cb3','cb4','cb5'].forEach(id=>{const el=document.getElementById(id);if(el)el.checked=false;});
  ['bk-e1','bk-e2','bk-e3'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='none';});

  // Generate captcha
  const a=Math.floor(Math.random()*12)+1, b=Math.floor(Math.random()*12)+1;
  captchaAnswer=a+b;
  document.getElementById('bk-cap-q').textContent=`${a} + ${b} = ?`;

  // Fill date card
  document.getElementById('bkdc-date').textContent=`${d} ${MONTHS[calMonth-1]} ${calYear}`;
  const MAX=SETTINGS.park.maxBookingsPerDay, open=MAX-bookedBy.length;
  document.getElementById('bkdc-meta').textContent=`${open} spot${open!==1?'s':''} remaining · ${bookedBy.length} of ${MAX} confirmed`;
  document.getElementById('bkdc-dots').innerHTML=
    Array.from({length:MAX},(_,i)=>`<div class="bkd-dot ${i<bookedBy.length?'taken':'open'}"></div>`).join('');

  // Open overlay and start animated queue
  document.getElementById('bk-overlay').classList.add('open');
  document.querySelectorAll('#bk-overlay .bk-step').forEach(s=>{s.classList.remove('active');s.style.display='none';});
  const s0=document.getElementById('bk-s0'); s0.style.display='block'; s0.classList.add('active');

  runLoadSequence(
    'bk-prog',
    'bk-load-msg',
    ['bkci-1','bkci-2','bkci-3','bkci-4'],
    [
      'Establishing secure connection…',
      'Verifying date availability…',
      'Loading capacity data…',
      'Fetching park schedule…',
      'Ready!'
    ],
    640,
    ()=>showStep('bk-s1')
  );
}

function closeBooking(){
  document.getElementById('bk-overlay').classList.remove('open');
  activeDate=null;
}

function showErr(id,msg){
  const el=document.getElementById(id); if(!el) return;
  el.textContent=msg; el.style.display='block';
  el.style.animation='none'; void el.offsetWidth; el.style.animation='shake .38s ease';
}

function step1to2(){
  const uname=document.getElementById('bk-uname').value.trim();
  const cap  =parseInt(document.getElementById('bk-cap-a').value);
  if(uname.toLowerCase()!==currentUser.toLowerCase())
    return showErr('bk-e1','Username does not match your login. Please enter it exactly.');
  if(isNaN(cap)||cap!==captchaAnswer)
    return showErr('bk-e1','Incorrect answer to the security question. Please try again.');
  document.getElementById('bk-e1').style.display='none';
  showStep('bk-s2');
}

function step2to3(){
  const all=['cb1','cb2','cb3','cb4','cb5'].every(id=>document.getElementById(id)?.checked);
  if(!all) return showErr('bk-e2','Please confirm all five points before continuing.');
  document.getElementById('bk-e2').style.display='none';
  document.getElementById('bks-guest').textContent=currentUser;
  document.getElementById('bks-date').textContent=activeDate
    ?`${activeDate.d} ${MONTHS[calMonth-1]} ${calYear}`:'—';
  showStep('bk-s3');
}

function doSubmit(){
  if(!activeDate||!currentUser) return;

  // Step 4: Transmission loading
  showStep('bk-s4');
  // Reset step4 checklist
  ['bksci-1','bksci-2','bksci-3','bksci-4'].forEach(id=>{
    const el=document.getElementById(id);
    if(el){ el.className='bkci'; }
  });
  document.getElementById('bk-prog2').style.width='0%';

  runLoadSequence(
    'bk-prog2',
    'bk-send-msg',
    ['bksci-1','bksci-2','bksci-3','bksci-4'],
    [
      'Encrypting booking data…',
      'Registering your request…',
      'Notifying park admin…',
      'Generating confirmation…',
      'Transmitted!'
    ],
    700,
    ()=>{
      // Save data
      const {key,bookedBy}=activeDate;
      const d=userData(currentUser);
      if(!d.pendingDates.includes(key)){ d.pendingDates.push(key); saveData(currentUser,d); }
      const [y,mo,day]=key.split('-').map(Number);
      const bD=SETTINGS.bookingDays;
      if(!bD[y])bD[y]={};if(!bD[y][mo])bD[y][mo]={};
      if(!bD[y][mo][day])bD[y][mo][day]={enabled:true,bookedBy:[],pendingBy:[]};
      if(!bD[y][mo][day].pendingBy)bD[y][mo][day].pendingBy=[];
      if(!bD[y][mo][day].pendingBy.includes(currentUser))
        bD[y][mo][day].pendingBy.push(currentUser);

      sendWebhook(SETTINGS.webhooks.regularBooking,{
        username:currentUser,date:key,type:'Standard Entry',
        age:currentAge,status:'⏳ PENDING — awaiting admin confirmation',
        openSpots:SETTINGS.park.maxBookingsPerDay-bookedBy.length,
      });

      // Step 5: Final verification
      showStep('bk-s5');
      runVerifySequence(()=>{
        // Done!
        const dateStr=`${activeDate.d} ${MON_S[calMonth-1]} ${calYear}`;
        document.getElementById('bk-done-body').textContent=
          `Your booking request for ${dateStr} has been submitted, ${currentUser}. It is now pending admin approval — you will be notified on Discord once confirmed.`;
        const ref=`HPR-${Date.now().toString(36).toUpperCase().slice(-6)}`;
        const refEl=document.getElementById('bk-done-ref');
        if(refEl) refEl.textContent=`Reference: ${ref}`;
        showStep('bk-s6');
        renderCal(); updatePendingBadge(); confetti(28);
      });
    }
  );
}

/* ── Generic loading sequence helper ─────────────────────── */
function runLoadSequence(progId, msgId, checkIds, msgs, interval, onDone){
  const prog=document.getElementById(progId);
  const msgEl=document.getElementById(msgId);
  if(prog) prog.style.width='0%';
  checkIds.forEach(id=>{
    const el=document.getElementById(id);
    if(el){ el.className='bkci'; const sp=el.querySelector('span'); if(sp){ sp.style.cssText=''; }}
  });
  let step=0, pct=0;
  const iv=setInterval(()=>{
    step++;
    pct=Math.min(pct+Math.floor(100/checkIds.length),100);
    if(prog) prog.style.width=pct+'%';
    if(msgEl&&msgs[step]) msgEl.textContent=msgs[step];
    // Mark previous done
    const prev=document.getElementById(checkIds[step-2]);
    if(prev) prev.className='bkci done';
    // Mark current active
    const cur=document.getElementById(checkIds[step-1]);
    if(cur) cur.className='bkci active';
    if(step>=checkIds.length){
      clearInterval(iv);
      const last=document.getElementById(checkIds[checkIds.length-1]);
      if(last) last.className='bkci done';
      if(prog) prog.style.width='100%';
      setTimeout(onDone,480);
    }
  },interval);
}

/* ── Final verification sequence ─────────────────────────── */
function runVerifySequence(onDone){
  const rows=[
    {row:'bkvd-1',ok:'bkvd-ok1'},
    {row:'bkvd-2',ok:'bkvd-ok2'},
    {row:'bkvd-3',ok:'bkvd-ok3'},
    {row:'bkvd-4',ok:'bkvd-ok4'},
  ];
  const msgs=[
    'Running final checks on your booking…',
    'Confirming identity…',
    'Locking in your date…',
    'Recording terms acceptance…',
    'All checks passed!'
  ];
  const msgEl=document.getElementById('bk-verify-msg');
  // Reset dots
  rows.forEach(r=>{
    const row=document.getElementById(r.row);
    if(row){ const dot=row.querySelector('.bkvd-dot'); if(dot) dot.className='bkvd-dot'; }
    const ok=document.getElementById(r.ok); if(ok) ok.textContent='';
  });
  let step=0;
  const iv=setInterval(()=>{
    if(msgEl&&msgs[step]) msgEl.textContent=msgs[step];
    if(step>0){
      const prev=rows[step-1];
      const prevRow=document.getElementById(prev.row);
      if(prevRow){ const dot=prevRow.querySelector('.bkvd-dot'); if(dot) dot.className='bkvd-dot done'; }
      const prevOk=document.getElementById(prev.ok); if(prevOk) prevOk.textContent='✓';
    }
    if(step<rows.length){
      const cur=rows[step];
      const curRow=document.getElementById(cur.row);
      if(curRow){ const dot=curRow.querySelector('.bkvd-dot'); if(dot) dot.className='bkvd-dot spinning'; }
    }
    step++;
    if(step>rows.length){
      clearInterval(iv);
      setTimeout(onDone,500);
    }
  },800);
}

/* ============================================================
   GOLDEN TICKET FLOW
   ============================================================ */
function wireGoldenFlow(){
  document.getElementById('btn-claim-golden')?.addEventListener('click',openGolden);
  document.getElementById('gt-close')?.addEventListener('click',closeGolden);
  document.getElementById('gt-overlay')?.addEventListener('click',e=>{
    if(e.target===document.getElementById('gt-overlay')) closeGolden();
  });
  document.getElementById('gt-submit')?.addEventListener('click',submitGolden);
  document.getElementById('gt-done-close')?.addEventListener('click',closeGolden);
}

function showGTStep(id){
  document.querySelectorAll('#gt-overlay .bk-step').forEach(s=>{s.classList.remove('active');s.style.display='none';});
  const el=document.getElementById(id); if(el){el.style.display='block';el.classList.add('active');}
}

function openGolden(){
  if(!currentUser) return;
  document.getElementById('gt-uname').value='';
  document.getElementById('gt-err').style.display='none';
  document.getElementById('gt-overlay').classList.add('open');
  showGTStep('gt-s0');
  setTimeout(()=>showGTStep('gt-s1'),1400);
}

function closeGolden(){
  document.getElementById('gt-overlay').classList.remove('open');
}

function submitGolden(){
  const u=document.getElementById('gt-uname').value.trim();
  if(u.toLowerCase()!==currentUser.toLowerCase()){
    const e=document.getElementById('gt-err');
    e.textContent='Username does not match.'; e.style.display='block'; return;
  }
  const d=userData(currentUser);
  d.goldenPending=true; saveData(currentUser,d);
  SETTINGS.goldenTicket.pendingBy=currentUser;
  sendWebhook(SETTINGS.webhooks.goldenTicket,{
    username:currentUser,date:SETTINGS.goldenTicket.bookingDate||'Open',
    type:'🏆 GOLDEN TICKET REQUEST',age:currentAge,
    label:SETTINGS.goldenTicket.label,
    status:'⏳ PENDING — awaiting admin confirmation'
  });
  showGTStep('gt-s2');
  buildGoldenTicket(); updatePendingBadge(); confetti(60,true);
}

/* ============================================================
   SCROLL REVEAL
   ============================================================ */
function initScrollReveal(){
  const obs=new IntersectionObserver(entries=>entries.forEach(e=>{
    if(e.isIntersecting){e.target.classList.add('on');obs.unobserve(e.target);}
  }),{threshold:0.07});
  document.querySelectorAll('.section').forEach(el=>obs.observe(el));
}

/* ============================================================
   CONFETTI
   ============================================================ */
function confetti(count=40,golden=false){
  const clrs=golden
    ?['#f0b800','#ffe066','#fff7cc','#ffffff','#ffd700']
    :['#c8380a','#e85020','#ffffff','#ffaa50','#00c47a'];
  for(let i=0;i<count;i++){
    const el=document.createElement('div'); el.className='confetti-piece';
    el.style.cssText=`left:${Math.random()*100}vw;background:${clrs[Math.floor(Math.random()*clrs.length)]};width:${Math.random()*8+4}px;height:${Math.random()*8+4}px;border-radius:${Math.random()>.5?'50%':'2px'};--dur:${(Math.random()*2+2.2).toFixed(2)}s;--delay:${(Math.random()*.7).toFixed(2)}s;--tx:${((Math.random()-.5)*180).toFixed(0)}px`;
    document.body.appendChild(el); setTimeout(()=>el.remove(),4500);
  }
}

/* ============================================================
   WEBHOOK
   ============================================================ */
async function sendWebhook(url,data){
  if(!url) return;
  const isGolden  =String(data.type).includes('GOLDEN');
  const isPending =String(data.status).includes('PENDING');
  const color     =isGolden?0xFFD700:isPending?0xFF9900:0xFF6A00;
  const embed={
    title:isGolden?'🏆 Golden Ticket Request!':'⏳ New Booking Request (Pending)',
    description:isGolden
      ?`A Golden Ticket request was submitted!\n\n> Move **${data.username}** to \`goldenTicket.claimedBy\` in \`settings.js\` to confirm.`
      :`New booking request — needs admin review.\n\n> Add **${data.username}** to \`bookedBy[]\` for **${data.date}** in \`settings.js\` to confirm.`,
    color,
    fields:[
      {name:'👤 Username',value:String(data.username),inline:true},
      {name:'📅 Date',    value:String(data.date),    inline:true},
      {name:'🎫 Type',    value:String(data.type),    inline:true},
      {name:'🎂 Age',     value:String(data.age||'?'),inline:true},
      {name:'📊 Status',  value:String(data.status),  inline:true},
    ],
    footer:{text:`Heide Park Roblox · ${new Date().toLocaleString()}`}
  };
  if(!isGolden&&data.openSpots!=null) embed.fields.push({name:'🪑 Open Spots',value:String(data.openSpots),inline:true});
  if(isGolden&&data.label) embed.fields.push({name:'✨ Event',value:String(data.label),inline:true});
  try{
    await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:'Heide Park Roblox',embeds:[embed]})});
  }catch(e){ console.warn('Webhook (CORS expected on GH Pages):',e.message); }
}
